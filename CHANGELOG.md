...
- Xtensa: implement atomic operations on ESP32 / ESP32-S3 in inline assembly (`s32c1i`), and route operations on PSRAM addresses, where `s32c1i` does not behave atomically, through a critical section. This keeps atomic CAS and the 64-bit and 128-bit lock-based fallbacks available on these chips even though the compiler cannot expose atomic CAS on them.
