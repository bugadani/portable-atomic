...
fn main() {
    ...
    if version.nightly {
        ...
        // https://github.com/rust-lang/rust/pull/128778 merged in Rust 1.83 (nightly-2024-09-29).
        if !version.probe(83, 2024, 9, 28) {
            println!("cargo:rustc-cfg=portable_atomic_old_miri");
        }
    }
    ...
    if version_pass && version.llvm >= 16 {
        println!("cargo:rustc-cfg=portable_atomic_llvm_16_or_later");
        if version.llvm >= 17 {
            println!("cargo:rustc-cfg=portable_atomic_llvm_17_or_later");
        }
    }
    ...
    match target_arch {
        "x86_64" => {
            ...
            if needs_target_feature_fallback(&version, Some(69)) {
                ...
                for &(enabled, name) in &rustflags.target_feature {
                    ...
                    match name {
                        b"cmpxchg16b" => cmpxchg16b = enabled,
                        _ => {}
                    }
                }
                emit_target_feature_fallback("cmpxchg16b", cmpxchg16b);
            }
        }
        "aarch64" | "arm64ec" => {
            ...
            if no_unstable {
                ...
                if needs_target_feature_fallback(&version, Some(61)) {
                    emit_target_feature_fallback("lse", lse);
                }
            }
            ...
            emit_target_feature_fallback("lsfe", lsfe);
        ...
        "arm" => {
            ...
            if needs_target_feature_fallback(&version, None) {
                ...
                for &(enabled, name) in &rustflags.target_feature {
                    ...
                    match name {
                        b"v8" | b"v8m.main" | b"v8.1m.main" | b"mve" | b"mve.fp" => {
                            dep!(v6, v7);
                        }
                        b"v7" => {
                            v7 = enabled;
        ...
        "riscv32" | "riscv64" => {
            ...
            if !version.probe(87, 2025, 2, 25) || needs_target_feature_fallback(&version, Some(94))
            {
                ...
                let check_zabha = check_za && version.llvm >= 19;
                let mut zacas = false;
                let mut zabha = false;
                let mut zaamo = false;
                // TODO: Handles cases where a specific target cpu
                ...
                for &(enabled, name) in &rustflags.target_feature {
                    ...
                    match name {
                        ...
                        _ if !check_za => {}
                        b"a" | b"rva23u64" => dep!(zaamo),
                        b"zabha" if check_zabha => {
                            zabha = enabled;
                            dep!(zaamo);
                        }
                        _ => {}
        ...
        "xtensa" => {
            // ESP32 / ESP32-S3 implement s32c1i, but it does not behave atomically
            // on addresses backed by external (PSRAM / data bus) memory, so the
            // compiler cannot expose atomic CAS on these CPUs. `src/imp/xtensa.rs`
            // implements the atomic operations itself and dispatches each one
            // based on the address at runtime; to select it we need to know which
            // of the two affected CPUs we are targeting. Infer from `target-cpu`,
            // or from the known Espressif target triples when no target-cpu is
            // specified.
            // ESP32-S2 has no s32c1i at all and is handled by the generic
            // no-CAS path, so it is intentionally absent here.
            if let Some(cpu) = rustflags.target_cpu {
                if cpu == "esp32" || cpu == "esp32s3" {
                    target_cpu_fallback(cpu);
                    println!("cargo:rustc-cfg=portable_atomic_xtensa_cas");
                }
            } else {
                let esp_cpu = match target {
                    ...
                    // ESP32-S2 does not have s32c1i, so it is not affected by the issue the same way.
